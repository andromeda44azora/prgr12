...
class SaistītaisSaraksts:
    def __init__(self):
        self.saraksts = []

    def pievienot_elementu(self, elements):
        self.saraksts.append(elements)

    def dzest_elementu(self, indekss):
        if 0 <= indekss < len(self.saraksts):
            del self.saraksts[indekss]
            print(f"Elements ar indeksu {indekss} ir veiksmīgi dzēsts.")
        else:
            print("Nederīgs indekss. Dzēšana neizdevās.")

    def nolasit_elementu(self, indekss):
        if 0 <= indekss < len(self.saraksts):
            print(f"Elements ar indeksu {indekss}: {self.saraksts[indekss]}")
        else:
            print("Nederīgs indekss. Nolasīšana neizdevās.")
